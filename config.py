...
# Каждая загрузка идёт в отдельном процессе (jobrunner). Пределы его сторожа:
# сколько ждать прогресса, прежде чем счесть источник зависшим (ретраи
# yt-dlp при этом уже исчерпаны или бесполезны); сколько может длиться
# склейка/перекодирование без событий; сколько ждать мягкой остановки,
# прежде чем добить процесс вместе с ffmpeg.
STALL_SEC = int(os.environ.get("YTG_STALL_SEC", "180"))
PROCESSING_TIMEOUT_SEC = int(os.environ.get("YTG_PROCESSING_TIMEOUT_SEC", "3600"))
CANCEL_GRACE_SEC = int(os.environ.get("YTG_CANCEL_GRACE_SEC", "15"))
# Файлы с googlevideo качать своим параллельным загрузчиком (racefd):
# DPI пропускает лишь часть соединений, и много коротких попыток находят
# живые за секунды. Выключатель — на случай, если он начнёт мешать.
YT_PARALLEL = os.environ.get("YTG_YT_PARALLEL", "1") == "1"
# Общий список заблокированных видеосерверов (hostban): сервер, к которому
# подряд BAN_FAILS соединений не прошло ни одно, все загрузки обходят
# BAN_TTL_SEC секунд (блокировки со временем меняются). Удача снимает бан.
